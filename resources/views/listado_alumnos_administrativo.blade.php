...
@section('header')
    ...
    @if(isset($crud->gruppo->descripcion))
        @if(Auth::user()->idescuelanormal == 1)
            en {{$crud->gruppo->descripcion}} ({{mb_strtolower($crud->gruppo->licenciatura)}})
        @else
...
<div class="modal fade" id="migrarGrupo" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    ...
    @if(isset($crud->certificacion))
    <a href="{{url('certificadosescuela', ['idnormal' => Auth::user()->idescuelanormal])}}" class="btn btn-primary ladda-button"  data-style="zoom-in"><span class="ladda-label"><i class="glyphicon glyphicon-print"></i>Imprimir Todos los Certificados</span></a>
    @else
